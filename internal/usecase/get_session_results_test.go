...
func TestGetSessionResultsUseCase_Execute(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		opRepo := &operationRepoMock{
			getPlayerAggFn: func(tx Tx, sID entity.SessionID) (map[entity.PlayerID]PlayerAggregates, error) {
				return map[entity.PlayerID]PlayerAggregates{
		...
		res, err := uc.Execute(GetSessionResultsQuery{
			SessionID: "s1",
		})
		if err != nil {
	...
	t.Run("session not found", func(t *testing.T) {
		...
		_, err := uc.Execute(GetSessionResultsQuery{
			SessionID: "s1",
		})
