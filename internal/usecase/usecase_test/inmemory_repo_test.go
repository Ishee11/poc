package usecase_test

import (
	"sync"

	"github.com/ishee11/poc/internal/entity"
	"github.com/ishee11/poc/internal/usecase"
)

type inMemoryOperationRepo struct {
	mu         sync.Mutex
	operations []*entity.Operation
}

func (r *inMemoryOperationRepo) Save(tx usecase.Tx, op *entity.Operation) error {
	r.mu.Lock()
	defer r.mu.Unlock()

	for _, existing := range r.operations {
		// защита по requestID (идемпотентность)
		if existing.RequestID() == op.RequestID() {
			return entity.ErrDuplicateRequest
		}
		// защита по ID (техническая)
		if existing.ID() == op.ID() {
			return nil
		}
	}

	r.operations = append(r.operations, op)
	return nil
}

func (r *inMemoryOperationRepo) GetByRequestID(tx usecase.Tx, requestID string) (*entity.Operation, error) {
	r.mu.Lock()
	defer r.mu.Unlock()

	for _, op := range r.operations {
		if op.RequestID() == requestID {
			return op, nil
		}
	}
	return nil, nil
}

func (r *inMemoryOperationRepo) GetLastOperationType(
	tx usecase.Tx,
	sessionID entity.SessionID,
	playerID entity.PlayerID,
) (entity.OperationType, bool, error) {

	r.mu.Lock()
	defer r.mu.Unlock()

	reversed := make(map[entity.OperationID]struct{})

	for _, op := range r.operations {
		if op.Type() == entity.OperationReversal && op.ReferenceID() != nil {
			reversed[*op.ReferenceID()] = struct{}{}
		}
	}

	for i := len(r.operations) - 1; i >= 0; i-- {
		op := r.operations[i]

		if op.SessionID() != sessionID || op.PlayerID() != playerID {
			continue
		}

		if op.Type() == entity.OperationReversal {
			continue
		}

		if _, ok := reversed[op.ID()]; ok {
			continue
		}

		return op.Type(), true, nil
	}

	return "", false, nil
}

func (r *inMemoryOperationRepo) GetSessionAggregates(
	tx usecase.Tx,
	sessionID entity.SessionID,
) (usecase.SessionAggregates, error) {
	r.mu.Lock()
	defer r.mu.Unlock()

	var buyIn, cashOut int64

	for _, op := range r.operations {
		if op.SessionID() != sessionID {
			continue
		}

		switch op.Type() {

		case entity.OperationBuyIn:
			buyIn += op.Chips()

		case entity.OperationCashOut:
			cashOut += op.Chips()

		case entity.OperationReversal:
			refID := op.ReferenceID()
			if refID == nil {
				continue
			}

			target := r.findByID(*refID)
			if target == nil {
				continue
			}

			switch target.Type() {
			case entity.OperationBuyIn:
				buyIn -= target.Chips()
			case entity.OperationCashOut:
				cashOut -= target.Chips()
			}
		}
	}

	return usecase.SessionAggregates{
		TotalBuyIn:   buyIn,
		TotalCashOut: cashOut,
	}, nil
}

func (r *inMemoryOperationRepo) GetByID(tx usecase.Tx, id entity.OperationID) (*entity.Operation, error) {
	r.mu.Lock()
	defer r.mu.Unlock()

	for _, op := range r.operations {
		if op.ID() == id {
			return op, nil
		}
	}
	return nil, nil
}

func (r *inMemoryOperationRepo) ExistsReversal(tx usecase.Tx, targetID entity.OperationID) (bool, error) {
	r.mu.Lock()
	defer r.mu.Unlock()

	for _, op := range r.operations {
		if op.Type() == entity.OperationReversal &&
			op.ReferenceID() != nil &&
			*op.ReferenceID() == targetID {
			return true, nil
		}
	}
	return false, nil
}

func (r *inMemoryOperationRepo) ListBySession(
	tx usecase.Tx,
	sessionID entity.SessionID,
	limit int,
	offset int,
) ([]*entity.Operation, error) {

	r.mu.Lock()
	defer r.mu.Unlock()

	// 1. фильтруем по sessionID
	filtered := make([]*entity.Operation, 0)

	for _, op := range r.operations {
		if op.SessionID() == sessionID {
			filtered = append(filtered, op)
		}
	}

	// 2. порядок: DESC (последние сверху)
	result := make([]*entity.Operation, 0)

	start := len(filtered) - 1 - offset
	if start < 0 {
		return result, nil
	}

	count := 0

	for i := start; i >= 0; i-- {
		if limit > 0 && count >= limit {
			break
		}

		result = append(result, filtered[i])
		count++
	}

	return result, nil
}

func (r *inMemoryOperationRepo) GetPlayerAggregates(
	tx usecase.Tx,
	sessionID entity.SessionID,
) (map[entity.PlayerID]usecase.PlayerAggregates, error) {

	r.mu.Lock()
	defer r.mu.Unlock()

	result := make(map[entity.PlayerID]usecase.PlayerAggregates)

	for _, op := range r.operations {
		if op.SessionID() != sessionID {
			continue
		}

		playerID := op.PlayerID()
		aggr := result[playerID]

		switch op.Type() {

		case entity.OperationBuyIn:
			aggr.BuyIn += op.Chips()

		case entity.OperationCashOut:
			aggr.CashOut += op.Chips()

		case entity.OperationReversal:
			refID := op.ReferenceID()
			if refID == nil {
				continue
			}

			target := r.findByID(*refID)
			if target == nil {
				continue
			}

			targetPlayer := target.PlayerID()
			targetAggr := result[targetPlayer]

			switch target.Type() {
			case entity.OperationBuyIn:
				targetAggr.BuyIn -= target.Chips()
			case entity.OperationCashOut:
				targetAggr.CashOut -= target.Chips()
			}

			result[targetPlayer] = targetAggr
			continue
		}

		result[playerID] = aggr
	}

	return result, nil
}

// внутренний helper (БЕЗ lock)
func (r *inMemoryOperationRepo) findByID(id entity.OperationID) *entity.Operation {
	for _, op := range r.operations {
		if op.ID() == id {
			return op
		}
	}
	return nil
}

// --- Session repo ---

type inMemorySessionRepo struct {
	mu       sync.Mutex
	sessions map[entity.SessionID]*entity.Session
}

func newSessionRepo() *inMemorySessionRepo {
	return &inMemorySessionRepo{
		sessions: make(map[entity.SessionID]*entity.Session),
	}
}

func (r *inMemorySessionRepo) FindByID(tx usecase.Tx, id entity.SessionID) (*entity.Session, error) {
	r.mu.Lock()
	defer r.mu.Unlock()

	return r.sessions[id], nil
}

func (r *inMemorySessionRepo) Save(tx usecase.Tx, s *entity.Session) error {
	r.mu.Lock()
	defer r.mu.Unlock()

	r.sessions[s.ID()] = s
	return nil
}

// --- Tx stub ---

type txManagerStub struct{}

func (m *txManagerStub) RunInTx(fn func(tx usecase.Tx) error) error {
	return fn(struct{}{})
}
