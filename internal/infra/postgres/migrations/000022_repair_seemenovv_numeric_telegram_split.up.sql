...
BEGIN
    ...
    WHERE provider = 'telegram'
      ...
      AND LOWER(COALESCE(username, '')) = 'seemenovv';
    ...
    WHERE ai.provider = 'telegram'
      ...
      AND ai.subject ~ '^[0-9]+$'
      AND LOWER(COALESCE(ai.username, '')) = 'seemenovv'
      AND duplicate_user.email LIKE 'telegram-%@telegram.invalid';

    IF duplicate_count = 0 THEN
        RETURN;
    END IF;
    IF duplicate_count <> 1 OR canonical_subject = target_subject THEN
        RAISE EXCEPTION 'ambiguous numeric Telegram identity repair for ishee@yandex.ru';
    END IF;
    ...
    WHERE provider = 'telegram' AND subject = target_subject AND user_id = target_user_id;
